                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control text-uppercase" name="sales_notes" placeholder="Memo / Catatan" value="<?= $sale->sales_notes ?>">
                            </div>
            ...
            <div class="card-body">
                <form method="post" action="<?= base_url('sales/sale/item/add') ?>">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3" id="dynamic-select-container">
                            <div class="form-group">
                                <label>Lokasi Inden</label>
                                <select name='indenWarehouse' class='select2bs4 form-control' id="subWarehouse">
                                    <!-- Opsi sub-gudang akan ditambahkan atau dihapus secara dinamis di sini -->
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3">
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php endif ?>
                                        <?php
                                        $retur_item = $db->table('return_item');
                                        $retur_item->select('warehouses.name as warehouse_name');
                                        $retur_item->join('warehouses', 'return_item.warehouse_id = warehouses.id', 'left');
                                        $retur_item->where('return_item.sale_item_id', $item->id);
                                        $retur_item = $retur_item->get();
                                        $retur_item = $retur_item->getFirstRow();

                                        if ($retur_item != NULL) {
                                            echo "<hr>";
                                            echo "<br>";
                                            foreach ($retur_item as $retur) {
                                                echo $retur->warehouse_name;
                                            }
                                        }
                                        ?>
                                    </td>
                                    <td class='text-right'>
                                        ...
                                        <?php
                                        $retur_item = $db->table('return_item');
                                        $retur_item->select('return_item.quantity');
                                        $retur_item->where('return_item.sale_item_id', $item->id);
                                        $retur_item = $retur_item->get();
                                        $retur_item = $retur_item->getResultObject();
...
<script>
    $(document).ready(function() {
        // Pilih gudang saat halaman dimuat
        var selectedWarehouseId = $("#warehouse").val();
        handleSubWarehouseOptions(selectedWarehouseId);

        // Ketika gudang dipilih
        $("#warehouse").change(function() {
            // Ambil nilai ID gudang yang dipilih
            var selectedWarehouseId = $(this).val();

            handleSubWarehouseOptions(selectedWarehouseId);
        });

        function handleSubWarehouseOptions(warehouseId) {
            if (warehouseId === "6") {
                $("#dynamic-select-container").show();
                $("#subWarehouse").html(`
                                    <option value=''>-- Pilih gudang inden --</option>
                                    ...
                                    <option value='9'>Tasikmalaya - Pasarwetan</option>
                                    `);
            } else {
