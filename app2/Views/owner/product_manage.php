...
<li class="breadcrumb-item"><a href="<?= base_url('/owner/products'); ?>">Barang</a></li>
                                ...
                                <div class="card-body">
                                    ...
                                    if($rowPercentegeThisLevel == NULL){
                                        ...
                                        if($statusApproveOwner == 1){
                                            ?>
                                            <a href="<?= base_url('owner/price/'.$rowPercentegeThisLevel->id.'/product/'.$product->id.'/status/2/set') ?>" onclick="return confirm('Yakin setujui harga <?= $l ?>.?')" class='btn btn-success btn-sm' title="Setujui">
                                                <i class='fa fa-check'></i>
                                            </a>
                                            <a href="<?= base_url('owner/price/'.$rowPercentegeThisLevel->id.'/product/'.$product->id.'/status/3/set') ?>" onclick="return confirm('Yakin tidak setujui harga <?= $l ?>.?')" class='btn btn-danger btn-sm' title="Tidak Setujui">
                                                <i class='fa fa-times'></i>
                                            </a>
                                            <?php
                                        }else{
                                            if($statusApproveOwner == 2){
                                                echo"<div class='badge badge-success'>".config("App")->product_price_statuses[$statusApproveOwner]."</div>";
                                            }elseif($statusApproveOwner == 3){
                                                echo"<div class='badge badge-danger'>".config("App")->product_price_statuses[$statusApproveOwner]."</div>";
                                            }else{
