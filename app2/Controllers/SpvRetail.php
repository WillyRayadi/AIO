...
class SpvRetail extends BaseController
{
    protected $session;
    ...
    public function products()
    {
        ...
        return view('spvretail/products', $data);
    }
    public function products_manage($id){
        ...
        return view("spvretail/product_manage",$data);
    }
    public function product_price_set($price, $product, $status){
        $this->productPriceModel
        ->where("id",$price)
        ->set([
            "approve_spv_retail_status" => $status,
            "spv_retail_id" => $this->session->login_id,
            "date_spv_retail_approve"   => date("Y-m-d")
        ])->update();
        ...
        if($status == 2){
            ...
            return redirect()->to(base_url('spv/retail/products/'.$product.'/manage'));
        }else{
            $this->session->setFlashdata('message_type', 'success');
    ...
    public function price_approval(){
        ...
        $prices->where("product_prices.approve_spv_retail_status",1);
        ...
        return view("spvretail/price_approval",$data);
    }
    public function sales(){
        ...
        return view("spvretail/sales",$data);
    }
    public function sales_manage($id){
        ...
        return view("spvretail/sales_manage",$data);
    }
}