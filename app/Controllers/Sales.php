...
class Sales extends BaseController
{
   ...
   public function export_id()
    ...
    public function sales_add(){
        ...
        if(config("Login")->loginRole == 7){
            if($this->session->login_id == 9) {
                return redirect()->to(base_url('owner/sales/test/'.$thisID.'/manage')); 
            }
    ...
    public function sales_save(){
        ...
        ->set([
            ...
            "sales_notes"=> strtoupper($sales_notes),
    ...
    public function test_sales_save() {
        $id = $this->request->getPost('id');
        $contact = $this->request->getPost('contact');
        ...
        if(config("Login")->loginRole == 7){
            if($this->session->login_id == 9) {
                return redirect()->to(base_url('owner/sales/test/'.$id.'/manage'));  
            }  
            return redirect()->to(base_url('owner/sales/'.$id.'/manage'));            
        }else{
            return redirect()->to(base_url('sales/sales/'.$id.'/manage'));
    ...
    public function ajax_test_sale_product_all()
    {
        $product = $this->request->getPost('product');
    ...
    public function sales_delete($id){
        ...
        $this->saleModel->where('sales.id', $id)
        ->set([
            'trash' => 1,
            'status'=> 6,
        ])->update();
        ...
        return redirect()->to(base_url('sales/sales/'));
    }
    
    private function saveSalesOrder($id)
    {
        ...
        $warehouses = $this->getWarehouseName($sales->warehouse_id);
    ...
    {
        $products = $this->productModel->select(["sku_number", "name"])->where('id', $productID)->first();
