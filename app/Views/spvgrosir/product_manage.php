...
<li class="breadcrumb-item"><a href="<?= base_url('/spv/grosir/products'); ?>">Barang</a></li>
    ...
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    <?php
                    for($l = 3; $l <= config("App")->productPriceLevel; $l++){
                        ?>
                        <?php
                                ...
                                <div class="card-body">
                                    ...
                                    if($rowPercentegeThisLevel == NULL){
                                        ...
                                        if($statusApproveSpvGrosir == 1){
                                            ?>
                                            <a href="<?= base_url('spv/grosir/price/'.$rowPercentegeThisLevel->id.'/product/'.$product->id.'/status/2/set') ?>" onclick="return confirm('Yakin setujui harga <?= $l ?>.?')" class='btn btn-success btn-sm' title="Setujui">
                                                <i class='fa fa-check'></i>
                                            </a>
                                            <a href="<?= base_url('spv/grosir/price/'.$rowPercentegeThisLevel->id.'/product/'.$product->id.'/status/3/set') ?>" onclick="return confirm('Yakin tidak setujui harga <?= $l ?>.?')" class='btn btn-danger btn-sm' title="Tidak Setujui">
                                                <i class='fa fa-times'></i>
                                            </a>
                                            <?php
                                        }else{
                                            if($statusApproveSpvGrosir == 2){
                                                echo"<div class='badge badge-success'>".config("App")->product_price_statuses[$statusApproveSpvGrosir]."</div>";
                                            }elseif($statusApproveSpvGrosir == 3){
                                                echo"<div class='badge badge-danger'>".config("App")->product_price_statuses[$statusApproveSpvGrosir]."</div>";
                                            }else{
