    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-blue">
                <h5 class="modal-title" id="staticBackdropLabel">Pengajuan Print</h5>
                <button style="color: white;" type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form method="POST" action="<?= base_url("add/submission/print") ?>" enctype="multipart/form-data">
                <input type="hidden" name="sales_id" class="form-control" value="<?= $sale->id ?>">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="admin_id">
                            <?php 
                                ...
                                $admins->where('id', $session->login_id);
                                $admins = $admins->get();
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="alasan" class="form-control" name="reason"></textarea>
                    </div>
                ...
                <tbody>
                    ...
                    foreach($items as $item){
                        ...
                        <tr>
                            ...
                            <td class='text-right'><?= $thisWarehouse->name ?></td>
                            <td class='text-right'>
...
<?php
    ...
    if($roles->delivery_order_buat != NULL): ?>
            ...
            <div class="card-body">
                <div class="tab-content" id="delivery-tabs-content">
                    ...
                    <?php foreach($deliveries as $delivery) : ?>
                        ...
                        <div class="tab-pane fade <?= ($d == 1) ? "show active" : "" ?>" id="delivery-tabs-<?= $d ?>-content" role="tabpanel">
                            <form method="post" action="<?= base_url('sales/deliveries/save') ?>">
                                <input type='hidden' name='id' value="<?= $delivery->id ?>">
                                <input type='hidden' name='sale' value="<?= $sale->id ?>">

                                <div class='form-group'>
                                    <label>Kendaraan</label>
                                    <select name='vehicle' class='form-control select2bs4' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                                ...
                                <div class="form-group">    
                                    <button type='submit' class='btn btn-success' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                                    ...
                                    <?php if ($delivery->print < 1): ?>
                                        ...
                                        if($sale->status > 5){
                                            ...
                                            ?>
                                            <a href="<?= base_url('sales/sales/'.$sale->id.'/deliveries/'.$delivery->id.'/delete') ?>" onclick="return confirm('Yakin hapus pengiriman ini.?')" class='btn btn-danger'>
                                                <i class='fa fa-trash'></i>
                                                Hapus
                                            </a>
                                            <?php
                            ...
                            <div class="table-responsive">
                                <table class='table table-bordered'>
                                    ...
                                    <tbody>
                                        <form method="post" action="<?= base_url('sales/deliveries/item/add') ?>">
                                            ...
                                            <tr>
                                                <td></td>
                                                <td>
                                                    <select class="form-control" name='item' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                                                        <?php
                                                        foreach($items as $item){
                                                            $thisProduct = $db->table("products");
                                                            $thisProduct->where("id",$item->product_id);
                                                ...
                                                <td>
                                                    <input type='number' name='qty' class='form-control' required placeholder='Kuantitas' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                                                </td>
                                                <td class='text-center'>

                                                    <button type='submit' class='btn btn-primary' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                                                        <i class='fa fa-plus'></i>
                                                        Tambah Item
                                                    </button>
                                                </td>
                                        ...
                                        foreach($delivery_items as $item){
                                            ...
                                            ?>
                                            <form action="<?= base_url('sales/deliveries/item/save') ?>" method="post">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type='number' name='qty' value="<?= $item->quantity ?>" class='form-control' required placeholder='Kuantitas' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                                                    </td> 
                                                    <td class='text-center'>
                                                     <button type='submit' class='btn btn-success' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                                                        <i class='fa fa-save'></i>
                                                    ...
                                                    if($sale->status > 5){
                                                        ...
                                                        ?>
                                                        <a href="<?= base_url('sales/sales/'.$sale->id.'/delivery_items/'.$item->id.'/delete') ?>" onclick="return confirm('Yakin hapus item dari pengiriman.?')" class='btn btn-danger'>
                                                            <i class='fa fa-trash'></i>
                                                            Hapus
                                                        </a>
                                                        <?php
                ...
                <?php if($sale->sent_date != NULL) : ?>
                    <div class="tab-pane fade <?= ($d == 0) ? "show active" : "" ?>" id="delivery-tabs-0-content" role="tabpanel">
                        <b>Data Pengiriman</b>
                        <hr>
                        <form action="<?= base_url('sales/sales/drive/save') ?>" method='post'>
                            <input type='hidden' name='id' value="<?= $sale->id ?>">
                            <div class='form-group'>
                                <label>Kendaraan</label>
                ...
                <div class="tab-pane fade <?= ($d == 0 && $sale->sent_date == NULL) ? "show active" : "" ?>" id="delivery-tabs-add-content" role="tabpanel">
                    <form method="post" action="<?= base_url('sales/deliveries/add') ?>">
                        <input type='hidden' name='id' value="<?= $sale->id ?>">
                        <div class='form-group'>
                            <label>Kendaraan</label>
                            <select name='vehicle' class='form-control' <?php if($sale->status > 5){ echo"disabled"; } ?>>
                        ...
                        <div class="form-group">
                            <label>Sopir</label>
                            <input type='text' name='driver' <?php if($sale->status > 5){ echo"disabled"; } ?> class='form-control' required placeholder="Nama Sopir">
                        </div>

                        <div class="form-group">
                            <label>Tanggal Dikirim</label>
                            <input type='date' name='sent_date' <?php if($sale->status > 5){ echo"disabled"; } ?> class='form-control' required>
                        </div>

                        <div class="form-group">
                            <label>Keterangan / Catatan</label>
                            <textarea name='warehouse_notes' <?php if($sale->status > 5){ echo"disabled"; } ?> placeholder="Tulis Keterangan / Catatan Di Sini" required class='form-control'></textarea>
                        </div>
