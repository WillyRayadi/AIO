...
<div class="card">
    ...
    <div class="card-body">
        <table class="table table-bordered table-striped" id="accounting">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Tgl. Transaksi</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#accounting').DataTable({
            processing: true,
            serverSide: true,
            ajax: '<?= base_url("ajax/accounting/sales") ?>',
            order: [],
            columns: [{
                    data: "number"
                },
                ...
                },
                {
                    data: null,
                    render: function(data, type, row) {
                        return '<a href="<?= base_url('accounting/sales_manage') ?>/' + row.id + '/manage" class="btn btn-success btn-sm" title="Kelola Pesanan Penjualan"><i class="fa fa-cog"></i></a>';
                    }
                }
            ]
        });
